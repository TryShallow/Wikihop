...
class Preprocesser:
    ...
    def preprocess(self):
        data_dir = "data_gen/"
        preprocess_graph_file_name = data_dir + self.file_name
        preprocess_elmo_file_name = data_dir + self.file_name + '.elmo'
        preprocess_glove_file_name = data_dir + self.file_name + '.glove'
        preprocess_extra_file_name = data_dir + self.file_name + '.extra'
    ...
    def doPreprocessForElmo(self, text_data, preprocess_elmo_file_name):
        if not os.path.isfile('{}.preprocessed.pickle'.format(preprocess_elmo_file_name)):
            elmoEmbedder = ElmoEmbedder(cuda_device=0)
...
if __name__ == '__main__':
    ...
    preprocesser = Preprocesser(file_name, logger, is_masked=is_masked,
            use_glove=use_glove, use_extra_feature=use_extra_feature, glove_file=glove_file)
