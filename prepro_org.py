...
class Preprocesser:
    def __init__(self, file_name, logger, is_masked=False):
        self.file_name = file_name
        self.dir_gen = "data_gen/"
        self.graph_file = self.dir_gen + self.file_name + ".graph_org_all.pickle"
        self.elmo_file = self.dir_gen + self.file_name + ".elmo_org_all.pickle"
    ...
    def doPreprocessForElmo(self, text_data):
        if not os.path.isfile(self.elmo_file):
            elmoEmbedder = ElmoEmbedder(cuda_device=-1
                                        )
    ...
    def doPreprocess(self, data_mb, mode, supports=None, ee=None):
        data_gen = []
...
file_name = "dev.json"
processer = Preprocesser(file_name, logger, is_masked=False)
